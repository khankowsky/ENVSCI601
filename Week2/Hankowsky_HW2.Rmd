...
tuition_long <- tuition_long %>%
  group_by(College) %>%
  mutate(diff = Tuition - lag(Tuition))

tuition <- tuition %>%
  mutate(diff = OutOfState - lag(Tuition))
...
tuition %>%
  ...
  labs(title = "Tuition Difference: Out-Of-State minus In-State",
...
tuition %>%
  ggplot() +
  geom_boxplot(aes(y = log_diff))

tuition %>%
  ggplot() +
  geom_histogram(aes(x = log_diff))
...
tuition_public_t <- tuition %>%
  filter(Type == "Public")

t1 <- t.test(log_outofstate, log_instate, data = tuition_public_t, paired = TRUE)
t1
...
tuition_instate_t <- tuition_long %>%
  filter(State == "InState")

t2 <- t.test(log_instate ~ Type, var.equal = TRUE, data = tuition_instate_t)
t2
...
tuition_outstate_t <- tuition_long %>%
  filter(State == "OutOfState")

t3 <- t.test(log_outofstate ~ Type, var.equal = FALSE, data = tuition_outstate_t)
t3
...
#ALL CODE I WROTE BEFORE I REALIZED THIS WAS A PAIRED T-TEST PROBLEM, 
#COULDNT BRING MYSELF TO DELETE IT
...
tuition_long %>%
  ...
  labs(title = "Boxplot of In-State vs Out-Of-State Tuition", 
       x = "") + 
  theme_classic()
...
ggplot() + 
  ...
  geom_histogram(data = tuition_long %>% 
                 ...
                 aes(x = Tuition, y = -..density.., fill = State), color = "black") + 
  scale_fill_discrete(name = "", labels = c("In-State", "Out-Of-State")) +
  labs(title = "Histogram of In-State vs Out-Of-State Tuition", 
       y = "Density") +
  theme_classic()
...
tuition_long %>%
  ...
  labs(title = "Boxplot of In-State vs Out-Of-State Tuition by University Type", 
       x = "") + 
  theme_classic()
...
ggplot() + 
  ...
  geom_histogram(data = tuition_long %>% 
                 ...
                 aes(x = Tuition, y = -..density.., fill = State), color = "black") + 
  scale_fill_discrete(name = "", labels = c("In-State", "Out-Of-State")) +
  labs(title = "Histogram of In-State vs Out-Of-State Tuition by University Type", 
       y = "Density") + 
  theme_classic() + 
  facet_wrap(~Type)
...
tuition_long %>%
  ...
  scale_x_discrete(labels = c("In-State", "Out-Of-State")) +
  scale_fill_discrete(name = "", labels = c("In-State", "Out-Of-State")) +
  labs(title = "Boxplot of In-State vs Out-Of-State Ln(Tuition)", 
       x = "", y = "Ln(Tuition)") + 
  theme_classic()
...
ggplot() + 
  ...
  geom_histogram(data = tuition_long %>% 
                 ...
                 aes(x = log_tuition, y = -..density.., fill = State), color = "black") + 
  scale_fill_discrete(name = "", labels = c("In-State", "Out-Of-State")) +
  labs(title = "Histogram of In-State vs Out-Of-State Ln(Tuition)", 
...
tuition_long %>%
  ggplot() + 
  geom_boxplot(aes(x = State, y = log_tuition, fill = Type)) + 
  scale_x_discrete(labels = c("In-State", "Out-Of-State")) +
  scale_fill_discrete(name = "University\nType") +
  labs(title = "Boxplot of In-State vs Out-Of-State Ln(Tuition) by University Type", 
       x = "", y = "Ln(Tuition)") + 
  theme_classic()
...
ggplot() + 
  ...
  geom_histogram(data = tuition_long %>% 
                 ...
                 aes(x = log_tuition, y = -..density.., fill = State), color = "black") + 
  scale_fill_discrete(name = "", labels = c("In-State", "Out-Of-State")) +
  labs(title = "Histogram of In-State vs Out-Of-State Ln(Tuition) by University Type", 
       y = "Density", x = "Ln(Tuition)") + 
  theme_classic() + 
  facet_wrap(~Type)
...
t1 <- t.test(log_tuition ~ State, var.equal = FALSE, data = tuition_public_t)
t1
...
t2 <- t.test(log_tuition ~ Type, var.equal = TRUE, data = tuition_instate_t)
t2
...
t3 <- t.test(log_tuition ~ Type, var.equal = FALSE, data = tuition_outstate_t)
t3
...
These data provide overwhelming evidence for a difference in in-state and out-of-state tuition in public schools (two-sided p-value < 0.00001 from an unequal variance two-sample t-test). The mean for out-of-state tuition for public universities was $8,710 greater than the in-state tuition (95% confidence interval: 0.36 to 0.53 dollars). 
...
#running the actual t-test
t.test(Iron ~ Supplement, var.equal = FALSE, data = mice_t) 
```

<br>

The variance for the two iron supplement groups are not equal, so an unequal variance two sample t-test was used. The histograms show that both iron supplement groups are relatively normally distributed. These data provide moderate evidence for a difference in percent iron retained based on the iron supplement type (two-sided p-value = 0.01 from an unequal variance two-sample t-test). The mean for the Fe4+ iron supplement was 2.24 percent greater than the mean for the Fe3+ iron supplement group (95% confidence interval: 0.57 to 3.90 %). 
