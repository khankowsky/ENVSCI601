...
## Basic 4.28 Darwin's Cross-Fertilization Experiment

### Cat Aliperta

## Basic 4.32 Therapeutic Marijuana

### Emily Fitzgerald

## Supplemental 4.30 Sunlight Protection Factor

### Jamie Beshore 

## Supplemental 4.31 Effect of Group Therapy on Survival of Breast Cancer Patients

### Craig Schaffer

## Master Problem: Gender differences on HW1 
...
* Mixed model ANOVA: random and fixed factors, often with nested factors
  ...
  + R’s Frequentist tests: lme4 & lme
  + R's Bayesian multilevel models: brms
  
* Repeated measures ANOVA

  + Repeated sampling of the same plots or individuals
  
  + Now handled as longitudinal models, usually as a type of mixed or multilevel model
...
* Design the experiment or survey after identifying the hypotheses and describe the statistical tests BEFORE collecting data (a priori vs . a posteriori hypotheses)
...
* Endeavor to create balanced designs with equal numbers of replicates at each combination of treatment and block levels
...
* Decide whether your factors are fixed or random: Are your treatment groups a random sample from a larger population, then a random effects model may be appropriate.

  + ANOVA tests for difference in means (fixed effect) or whether σ^2^= 0 (random effect) or both (mixed model or multilevel ANOVA)
...
* Avoid pseudoreplication (Hurlbert 1984)
  ...
  + Pseudoreplication only occurs after an incorrect analysis has been presented: if there is no analysis presented, then there can be no pseudoreplication
...
* Set the alpha level for hypothesis tests (i.e., the critical values) in advance. Tests and hypothesis, as far as possible, should be specified in advance. A priori hypotheses, if a small subset of possible tests, can be tested at the experiment-wise alpha level, usually α=0.05. Mead and Hurlbert disagree with specifying the alpha level in advance. Scientists must decide what p-value they consider a meaningful effect. Gelman doesn't use p-values. He regards any estimate less than 2 standard errors form 0 as being uncertain.
...
* Transform the data to correct unequal spread
  ...
  + Note that Zuur in his many R-based books rarely if ever transforms data, preferring to use analyses like generalized linear models that assume non-normal error structure
...
* Examine the data for outliers, but never remove outliers without strong justification
  ...
  1. If the conclusion remains the same, leave the potential outlier in, unless it has caused a major violation in assumptions
...
* Many results with low p-values are trivial ecologically (environmentally, chemically or socially)

  + Most null hypotheses (μ1 = μ2) are false and the p-value is often dependent on the sample size
  
  + Gelman never reports p-values in his statistics books. He reports the effect and standard error, and if the effect is within 2 standard errors of 0, Gelman regards the result as uncertain.
