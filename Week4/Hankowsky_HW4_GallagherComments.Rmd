...
---
<span style="color: magenta"> **General Comments [Gallagher's comments in magenta]:** I changed your read.csv call to match my folders and your file knitted beautifully (I just spent 30 min getting another student's code to knit). I didn't take off points for it, but on two problems you used the Chapter 6 Tukey HSD when the problems called for the Tukey LSD, which is just a t test with a pooled sd. **Basic score (5+4.5)/10, Supplemental Score (2.5+2.5)/5 Master 4.5/5 Total score 19/10** </span>
...
<span style="color: magenta"> **Basic 5.23_3rd ** Great analysis!  The traditional test for homogeneity of variance is the car::leveneTest, but your analysis was perhaps better than the low-power Levene's test, which showed a relatively high p-value. Even if Levene's test had a low p-value, none of the standard transformations would improve the unequal variance shown in your residual vs. fitted value plot. Your summary is great. The answer posed in the title of the question is a trick question. While the ANOVA results would indicate heterothermy, Barricks & Showers (course materials/reprints) conclude that the range of variation was in the modern homeothermic range and that T. res was warm blooded.** score 5/5** </span>
...
<span style="color: magenta"> **Basic 5.25_3rd ** Your analysis was very good, but with one big exception. The income data, as most income data do, cry out for a log transformation. car::leveneTest of the untransformed and transformed data show the need as does your residual vs. fitted value plot, The normal Q-Q plot which should be a straight line looks like a hockey stick. The density plots are all highly right skewed. With the log transform, the data look fine, and the assumption of homoscedasticity is met. The linear contrasts are ok, but in Chapter 5, the Sleuth authors wanted you to use just straight t tests with pooled sd. pairwise t.test does that and multcomp::glht does the linear contrast for the 4 comparisons. Tukey HSD is a great multicomparison test, but Sleuth doesn't introduce it until Chapter 6.   ** score 4.5/5** </span>
...
<span style="color: magenta"> **Supplemental 5.24_2nd ** Excellent answer. As a Chapter 5, not 6 problem, the Sleuth authors would prefer that you do the t.tests with no adjustment of p values for multicomparisons. You used Tukey's HSD, which is a very good multiple comparisons procedure. pairwise.t.test and multcomp::glht will solve for the p-values and estimates and CI's for the linear contrasts using a pooled sd. ** score 2.5/2.5** </span>
...
cck <- read.csv("../data/ex0525_2nd.csv",head=T)
...
<span style="color: magenta"> **Supplemental 5.25_2nd** Excellent answer. But, as a chapter 5 problem the soltuion the Sleuth authors were hoping for was the t.test with pooled sd, with no adjustment for multiple comparisons. This is called the Tukey Least Significant Difference test and can be done with pairwise.t.test and multcomp::glht. ** score 2.5/2.5** </span>
...
<span style="color: magenta"> **Master Graphical ANOVA** You can really see the positive skew crying out for a log transform in the residual plot. I'll attach a graphical ANOVA that I had a professional graphic artist modify for a paper I'm writing. The data in that paper were log transformed. ** score 4.5/5** </span>
